...
class ExperimentRunner:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            n_events_per_scenario: Target events per scenario (default 100)
    ...
    def run_all_scenarios(self) -> dict:
        ...
        LOGGER.info(f"Running {len(scenarios)} scenarios × {self.n_runs} runs = {len(scenarios) * self.n_runs} total runs")
    ...
    def _run_scenario_iteration(self, scenario: Scenario, run_num: int):
        ...
        # 7. Wait for N events (poll Kafka periodically)
        LOGGER.info(f"Waiting for {self.n_events} events...")
        self._wait_for_events(scenario)
    ...
    def _start_services(self, scenario: Scenario):
        ...
        try:
            ...
            if result.returncode != 0:
                raise RuntimeError(f"docker-compose up failed: {result.stderr.decode()}")
    ...
    def _wait_for_events(self, scenario: Scenario, timeout: int = 600):
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                # Simple parsing: look for message count in output
                output = result.stdout.decode()
                # This is approximate - just check if we have activity
                
                event_count = self._check_collector_file_growth()
                
                if event_count >= self.n_events:
                    LOGGER.info(f"Reached {event_count} events, stopping experiment")
                    break
                ...
                if no_progress_time > 60:
                    LOGGER.warning(f"No new events for 60 seconds, giving more time...")
                    no_progress_time = 0
                
                LOGGER.info(f"Event count: {event_count}/{self.n_events}")
        ...
        if event_count < self.n_events:
            LOGGER.warning(f"Timeout: only {event_count}/{self.n_events} events collected")

    def _check_collector_file_growth(self) -> int:
        """Estimate event count from collector files"""
        total_lines = 0
    ...
    def _run_event_processor(self, scenario: Scenario, run_num: int) -> Path:
        ...
        try:
            result = subprocess.run(
                ...
                cwd=event_processor_dir,  # FIX: Changed from using -m to direct cwd
                capture_output=True,
                timeout=60,
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help='Target events per scenario (default: 100)'
    )
    parser.add_argument(
        '--nRuns',
        type=int,
