...
Portions of the text have been revised with the assistance of an AI-based grammar and style tool. AI-based tools were also used for supporting code development.
...
The remainder of this paper is organised as follows. Section~\ref{sec:problem} describes the problem context and outlines our research questions and overall approach. Section~\ref{sec:related_work} provides the motivation for the architectural tactics and evaluation methods applied in this work. Section~\ref{sec:use_case} derives the main use cases and quality attribute scenarios that drive the architecture. Section~\ref{sec:design} presents the resulting system architecture and discusses key design trade-offs. Section~\ref{sec:formal_verification} reports the formal verification results obtained with UPPAAL, and Section~\ref{sec:evaluation} presents the empirical evaluation under controlled fault injection. Finally, Section~\ref{sec:conclusion} concludes the paper and highlights future work.
...
\subsection{Experiment design} \label{sec:experiment_design} The planning phase of the experiment determines how the tests (trials) are organized to maximize efficacy and align with the intended statistical analysis.
...
These analyses confirmed the system's responsiveness and demonstrated that the architectural design successfully addresses critical requirements, ensuring fulfillment of key quality attributes such as System Availability (QA-1), Production Modifiability (QA-2), and System Scalability (QA-3).
...
\subsubsection{Limitations and Improvement Opportunities}
Although the measured latencies satisfy the stated requirements, the results also highlight potential improvement opportunities. First, scheduler computation dominates end-to-end latency across all event types, suggesting that further performance gains should focus on reducing scheduling overhead (e.g., incremental plan updates, caching of stable machine capabilities, or more efficient assignment heuristics). Second, the experiment revealed saturation under heavy fault load, where corrective actions (e.g.\ blade swaps) may be impossible when too few machines remain available. A practical mitigation would be to introduce explicit `degraded mode' behaviour that prioritises essential production goals, surfaces capacity shortages to operators, and triggers controlled backpressure (e.g.\ slowing intake or rescheduling demand) rather than attempting infeasible plan adjustments.
